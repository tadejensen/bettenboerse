...
class Reservation2(db.Model):
    ...
    shelter_id = db.Column(db.ForeignKey("shelter.id"), primary_key=True)
    user_id = db.Column(db.ForeignKey("user.id"), primary_key=True)
    date = db.Column(db.String(50))
    user = relationship("User", back_populates="shelters")
...
class Shelter(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    address = db.Column(db.String)
    users = relationship("Reservation2", back_populates="shelter")
...
class User(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String)
    shelters = relationship("Reservation2", back_populates="user")
